...
class sales_transaction:
    ...
    """===METHODS FOR MANAGING THE CUSTOMER,PRODUCT,ORDERS & DELIVERY-ORDERS==="""
            ...
            while True:
                ...
                if chose == 1:
                    ...
                    data = {'customer': self.customer,
                            'order_lines': [i for i in oredrline.values()],
                            'order_date': current_date,
                            'order_total_amount': sum([i['subtotal'] for i in oredrline.values()]),
                            'state': 'Draft'}
                elif chose == 2:
                    break
                else:
    ...
    def create_order(self, values):
        ...
        self.sale_order_record.update({so_key: values})
        return so_key
    ...
    def draft_state(self):
        sale_order = self.sale_order_record[self.choice_sale_order]
        print(sale_order)
        for var_draft in sale_order['order_lines']:
    ...
    def conform_state(self):
        sale_order = self.sale_order_record[self.choice_sale_order]
        print(f'This is for confirm {sale_order}')
        for var_conform in sale_order['order_lines']:
    ...
    def cancel_state(self):
        sale_order = self.sale_order_record[self.choice_sale_order]
        for var_cancel in sale_order['order_lines']:
    ...
    def prepare_delivery_order(self, sale_order_id):
        """
        :param sale_order_id: This is parameter which created in prepare_delivery_order method for checking the sale
        order id weather it is available or not if available then get the sale order id from the sale order record
        and store back into the order variable
        :return:
        """
    ...
    def create_delivery_order(self,values):
        ...
        """
        :param values: This is the parameter which is created in the create delivery order method for saving the return
        values from the prepare_delivery_order method and give it back the return stored values in the delivery_order
        with update.
        :return:
        """
    ...
    def validate_delivery_order(self):
        ...
        if order_id in self.delivery_order:
            ...
            for order_line in sale_order_data.get('order_lines'):
                ...
                self.products[order_line['product_sku']] -= order_line['quantity']
