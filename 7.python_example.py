...
fieldnames = ['Order No', 'customer', 'SKU', 'Qty', 'Price', 'Address1', 'Address2', 'City', 'Country']

with open('7.python_example.csv', 'w') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(fieldnames)

    for key, value in orders.items():
       for v in value.get('Products'):
          writer.writerow([key,
                           value.get('customer').get('name'),
                           v.get('sku'),
                           v.get('qty'),
                           v.get('price'),
                           value.get('customer').get('address 1 '),
                           value.get('customer').get('address 2'),
                           value.get('customer').get('city'),
                           value.get('customer').get('country')])
csvfile.close()